...
from ShoppingOnline.settings import EMAIL_HOST_USER, SERVER_HOST, SERVER_PORT
...
def send_email_activate(username, receive, u_token):
    ...
    subject = '%s ShoppingOnline Activate' % username
...
def get_total_price():
    ...
    return float("%.2f" % total)   #'{:.2f}'.format(total)